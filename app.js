...
const CART_STORAGE_KEY = "dinner_product_cart_no_price_v1";
...
function categoryCount(categoryId) {
  ...
  return category.dishes.reduce((sum, dish) => {
    return sum + (cart[dish.id] || 0);
  }, 0);
}
...
function visibleDishes() {
  ...
  return favoriteOnly ? list.filter(dish => dish.favorite) : list;
}
...
function renderCategories() {
  $("categorySidebar").innerHTML = menuData
    .map(category => {
...
function renderDishes() {
  ...
  const title = searchText
    ? "搜索结果"
    : menuData.find(c => c.id === activeCategory)?.name || "菜单";
            ...
            <div class="dish-body">
              ...
              <div class="dish-bottom">
                ...
                <div class="counter">
                  ${
                    qty
                      ? `<button class="icon-btn minus" data-action="minus" data-id="${dish.id}">−</button><span class="qty">${qty}</span>`
                      : ""
                  }
...
function renderCartFab() {
  ...
  $("cartSubText").textContent = count ? "已选好，点这里确认" : "点 + 加入今晚菜单";
}
...
function renderCartList() {
        ...
        return `
          <div class="cart-row">
            <div>
              ...
              <div class="cart-note">数量 × ${qty}</div>
            </div>
  ...
  $("totalPrice").textContent = `${cartCount()} 份`;
